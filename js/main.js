...
const bindKeyboard = () => {
  window.addEventListener("keydown", (event) => {
    if (
      event.key in _DO_MOVEMENT_VECTOR &&
      _BLOCK_MOVEMENTS[event.key] !== _DIRECTION
    ) {
...
// Fabrica de Corpo e Comida da Snake
...
const update = (snake, foods, positions, canvas) => {
  ...
  const nextHead = snakePartFactory(
    head.x + movementVector[0] * _BASE_SIZE,
    head.y + movementVector[1] * _BASE_SIZE
  );
...
const renderInfo = (snake, ctx) => {
  ...
  ctx.fillText(
    `Time: ${minutes}m ${ELAPSED_TIME_IN_SECONDS % 60}s`,
    _BASE_SIZE,
    _BASE_SIZE * 2
  );
  ...
  ctx.fillText(
    `Difficult: ${_BASE_DELAY - _CURRENT_DELAY}`,
    _BASE_SIZE,
    _BASE_SIZE * 5
  );
};
...
const startGameTimer = (foods, snake, positions) =>
  setInterval(() => {
    ...
    if (
      foods.length < _MAX_FOODS_ON_GAME_AT_SAME_TIME &&
      ELAPSED_TIME_IN_SECONDS %
        parseInt(random(_MIN_FOOD_SPAWN_TIME, _MAX_FOOD_SPAWN_TIME), 10) ===
        0
    ) {
      generateRandomFood(foods, snake, positions);
    }
